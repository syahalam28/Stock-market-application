...
foreach ($stocks as $stock) {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=$curlsymbol&apikey=$key&outputsize=compact",
    ...
    if($err){
    	...
    	// Berfungsi untuk melihat apakah data berhasil ditambahkan ke dalam database yang diambil dari api dengan $response
        echo $response;
        ...
        foreach ($dates as $date) {
        	// Check ketersediaan data pada stock_daily_values berdasarkan tanggal/date
		    ...
		    // Jika tanggal belum di update maka akan ditambahkan sebuah data baru dari API
		    if($count == 0){

...
foreach ($stocks as $stock) {
    ...
    curl_setopt_array($curl, array(
        CURLOPT_URL => "https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=$curlsymbol&apikey=$key&outputsize=compact",
    ...
    if($err){
        ...
        $dates = array_keys($data['Weekly Time Series']);
        foreach ($dates as $date) {
        	// Check the values exists in stock_daily_values table with trade date
		    $sql = "SELECT * FROM stock_weekly_values WHERE stockid=:stockid AND trade_date=:trade_date";
		    $result = $db->prepare($sql);
		    ...
		    if($count == 0){
		    	...
		    	if(isset($data['Weekly Time Series'][$date]) & !empty($data['Weekly Time Series'][$date])){
		                if($data['Weekly Time Series'][$date]['1. open'] != '0.0000'){

		                    // Insert into stock_daily_values table
		                    $dailysql = "INSERT INTO stock_weekly_values (stockid, price_open, price_high, price_low, price_close, volume, trade_date) VALUES (:stockid, :price_open, :price_high, :price_low, :price_close, :volume, :trade_date)";
		                    $dailyresult = $db->prepare($dailysql);
		                    $values = array(':stockid'      => $stockid,
		                                    ':price_open'   => $data['Weekly Time Series'][$date]['1. open'],
		                                    ':price_high'   => $data['Weekly Time Series'][$date]['2. high'],
		                                    ':price_low'    => $data['Weekly Time Series'][$date]['3. low'],
		                                    ':price_close'  => $data['Weekly Time Series'][$date]['4. close'],
		                                    ':volume'       => $data['Weekly Time Series'][$date]['5. volume'],
		                                    ':trade_date'   => $date
		                                    );
